...
Firstly we had Typed method has body, which states that it is possible to
fetch the body of a function given the $mtype$ function. Since $mtype$ and
and $mbody$ are symmetrical on their definition, this is straightforward.
...
$mtype_R$ since it is used the negation $mtype_R$ on the definitions.
...
at the superclasses. Which is also straightfoward by the definition of
$override$ and $override_R$. In the case of \cref{lemma:ref-type} it 
...
given the return of $mtype$ or $mtype_$ and $mbody$ or $mbody_R$.
...
And finally we state The Progress and Preservation theorems.
