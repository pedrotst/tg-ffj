...
\section{Syntax}
The Syntax of \gls{FFJ} is a straightfoward \gls{FOP} extension of \gls{FJ}. Due to the lack
...
The syntax of \gls{FFJ+} constructs is given at Table~\ref{abstractsyntax}. The metavariables
...
Method override in both \gls{FFJ} and \gls{FFJ+} is basically method rewriting. 
...
Notice that these two definitions greatly differs from \gls{FFJ} to \gls{FFJ+}. In \gls{FFJ} $mtype$ would
...
The typing and computation rules for expressions are elided since are the same as \gls{FJ}. An environment
